...
thread1 = threading.Thread(target= (lambda: a.transmit("M3 3->1", 3, 1, 12))) 
thread2 = threading.Thread(target= (lambda: a.emit("M1", 1, [1,2,3], [4, 4, 2], [3, 3, 3], [0, 0, 0])))
thread3 = threading.Thread(target= (lambda: a.transmit("M1 1->3", 1, 2, 0)))  
thread4 = threading.Thread(target= (lambda: a.transmit("M3 2->3", 2, 3, 4))) 
thread5 = threading.Thread(target= (lambda: a.emit("M2", 2, [1,2,3], [6, 2, 6], [5, 5, 5], [0, 0, 0]))) 
thread6 = threading.Thread(target= (lambda: a.transmit("M3 1->2", 1, 2, 6))) 
thread7 = threading.Thread(target= (lambda: a.transmit("M3 2->3", 2, 3, 8))) 
thread8 = threading.Thread(target= (lambda: a.emit("M3", 3, [1,2,3], [2, 6, 4], [7, 7, 7], [0, 0, 0]))) 
thread9 = threading.Thread(target= (lambda: a.transmit("M3 1->2", 1, 2, 14)))
